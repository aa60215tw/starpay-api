...
//	   		$data = array_merge( $tpl, $data );
        ...
        if($options[ 'status' ] == $this->InternalServerError() ){
            ob_clean();
            ...
            echo "<form name=\"ShowForm\" method=\"post\" action=\"".REEOR_URL."\" target=\"_self\">";
            echo "<input type=\"hidden\" name=\"message\" value=\"".$data["message"]."\"/>";
            echo "<input type=\"hidden\" name=\"message_code\" value=\"".$data["message_code"]."\"/>";
            echo "</form></body></html>";
            ob_end_flush();
            exit();
        }
		...
		$statusHeader = 'HTTP/1.1 ' . $tpl[ 'c' ] . ' ' . $tpl[ 'm' ] ;
		header( $statusHeader );
		header( 'Content-type: ' . $options[ 'contentType' ] );//將傳送資料轉json格式，但會有空格所以先拿掉
		ob_end_clean();
