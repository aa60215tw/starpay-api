...
class DbHero
{
	...
	*	@return string sql sentence.
	*/
	private function _sqlQueryBuilder( $select = null, $from = null, $where = null, $limit = null, $offset = null, $join = null, $leftJoin = null, $rightJoin = null, $order = null, $group = null, $like = null, $having = null ,$for_update = null)
    {
	...
	*/
    ...
    /**
     * Create Page sql statement.
     * @param int $pageNo
     * @param int $pageSize
     * @return string
     * @throws Exception
     */
    public function createPagingSql($pageNo = 1, $pageSize = 10) {
        $sql = "";
        if ((is_int($pageNo) || is_numeric($pageNo)) && (is_int($pageSize) || is_numeric($pageSize))) {
            ...
            throw new Exception("Page size and page number should be integer or numeric string.", 1);
        }

        return $sql;
    }

    public function insert_select( $table1, $table2, $field1='', $field2='', $where='')
    {
        $sql = '';
        if (is_string($table1) && is_string($table2) && ($table1 && $table1) != null)
            $sql .= "INSERT INTO $table1";
        else
            throw new Exception("Unsupport the table input data type.", 1);

        if (is_string($field1) && is_string($field2) && ($field1 && $field2) != null)
            $sql .= " ($field1) SELECT $field2 FROM $table2";
        else
    ...
    public function for_update($for_update)
    {
        if(is_bool($for_update))
            $this->_for_update = $for_update;
        else
            throw new Exception("select is fail.", 1);
    }
