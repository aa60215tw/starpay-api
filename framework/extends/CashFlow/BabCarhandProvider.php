...
class BabCarhandProvider extends CashFlowProvider implements CashFlowProviderImp
{
    protected $md5_class;
    ...
    public function send_api($data)
    {
        ...
        $msg = '抢单，送往上游字段:' . json_encode($jsapi_array, 320);
        ...
        $output = $this->shared->curl($data['address'], $jsapi_array, '抢单', 'BabCarhand', 'post',$header, 'off', $curlTimeOut);
        $output = trim($output);
        $output_array = json_decode($output, true);

        $path_up_log = new LoggerHelper('path_up_log', PATH_REQUEST_LOG);
        $msg = '抢单，上游订单号:' . $data['upstream_order_number'] . '\r\n上游返回字段:' . $output;
        ...
        if ($output_array['data']['code'] != '0') {
            ...
            $error_treat = $this->error_treat('抢单', 'BabCarhand', $output, $data, 10, $output_array['err_msg']);
            if (!$error_treat['msg']) {
                $this->i++;
    ...
    public function getOrder($orderList = array())
    {
        ...
        $output = $this->shared->curl($orderList['query'], $jsapi, '抢单', 'BabCarhand', 'post', [], 'off', $curlTimeOut);
        $output_array = json_decode($output, true);
        ...
        unset($output_array['data']["sign"],$output_array['data']["code"],$output_array['data']["msg"]);
