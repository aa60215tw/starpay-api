...
$call = "抢单test呼叫callback成功, " . $data. ", JSON : ".json_encode($data). ", 呼叫IP：". $get_ip;
$log_call->warn($call);

if (empty($data)) {
    $fai = "抢单test-交易失败" . json_encode($data);
    $log_fail->warn($fai);
    echo 'error';
    exit;
}
...
if($pool_records == "order_not_find"){
    $fai = "抢单test-没有这笔订单编号" . json_encode($data). "呼叫IP：". $get_ip;
    $log_fail->warn($fai);
    echo 'success';
    exit;
}
...
if($pool_records == "pool_not_find"){
    $fai = "抢单test-水池查找失败" . json_encode($data). "呼叫IP：". $get_ip;
    $log_fail->warn($fai);
    echo 'error';
    exit;
}
...
if ($sign != $callback_sign) {
    $fai = "抢单test-验签失败" . json_encode($data). "呼叫IP：". $get_ip;
    $log_fail->warn($fai);
    echo 'error';
    exit;
}
...
if (!$orderList) {
    $fai = "抢单test-订单更新失败" . json_encode($data). "呼叫IP：". $get_ip;
    $log_fail->warn($fai);
    echo 'error';
    exit;
}

$str = "抢单test-交易成功！订单号：" . $data["cusordernum"] . json_encode($data). "呼叫IP：". $get_ip;
